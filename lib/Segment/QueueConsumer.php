...
abstract class Segment_QueueConsumer extends Segment_Consumer {
  ...
  public function flush() {
    ...
    while($count > 0 && $success) {
        ...
        if ($this->check_max_request_size) {
            ...
            $payload = $this->encodedPayload($batch);
       ...
       */
      $batch = array_splice($this->queue, 0, min($batch_size, $count));
      $success = $success && $this->flushBatch($batch);
   ...
   * @return string
   */
  protected function encodedPayload($batch) {
      return json_encode($this->payload($batch));
  }

  /**
   * Flushes a batch of messages.
   * @param  array $batch set of calls to flush
   * @return bool
   */
